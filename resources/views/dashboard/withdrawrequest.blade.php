        ...
        <div class="m-content mx-2 mx-lg-3 px-2 px-lg-0 pb-4">
            ...
            <div class="top-contnt row mt-3 justify-content-between mb-3">
                <div class="col-md-auto col-12">
                    ...
                    <h3 class="text-dark fw-bold">Withdraw Requests</h3>
                </div>
            ...
            <div>
                <div class="table-filter mt-4">
                    ...
                    <div class="table-responsive">
                        <table class="table fil-tb" style="vertical-align: middle; overflow-x: auto;">
                            <thead>
                                <tr>
                                    <th scope="col">ID</th>
                            ...
                            <tbody>
                                @foreach ($withdraws as $t)
                                    <tr class="invest-tb">
                                        <td class="table-srt">{{ $t->id }}</td>
                                        <td class="contry"><a
                                                href="{{ route('user.bank.details', ['id' => $t->user->id]) }}">
                                                {{ $t->user->username }} </a></td>
                                        <td>{{ $t->status }}</td>
                                        <td>{{ $t->amount }} $ </td>
                                        <td>{{ (new DateTime($t->created_at))->format('F j, Y, g:i a') }}</td>
                                        <td>
                                            <a href="#" data-id="{{ $t->id }}"
                                                class="action-delete btn btn-outline-danger">Reject</a>
                                            <a href="#" data-id="{{ $t->id }}"
                                                class="action-accept btn btn-outline-success">Approve</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
@section('additonal-script')
    ...
    <script>
        $(document).ready(function() {
            ...
            function sendingrequest(wid, action) {
                $.ajax({
                    url: '/withdraw-request-action',
                    method: 'POST',
                    data: {
                        wid: wid,
                        action: action
